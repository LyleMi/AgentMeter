...
func timeLines(overview agentmodel.Overview, width int, tab timeTab) []string {
	...
	switch tab {
	case timeTabSources:
		appendTimeSourceLines(writer, overview.AgentTimeUsage)
	case timeTabTools:
		appendTimeToolLines(writer, overview.ToolTimeLeaders)
	case timeTabSessions:
		appendTimeSessionLines(writer, overview.SlowSessions)
	default:
		writer.lines = appendTimeCompositionLines(writer.lines, overview)
		appendTimeAttributionSummaryLines(writer, overview)
	}
...
func appendTimeAttributionSummaryLines(w *fittedLineWriter, overview agentmodel.Overview) {
	w.append("", bold("Source Time Attribution"))
	appendTimeSourceRows(w, rankedAgentTimeUsage(overview.AgentTimeUsage), 5)
	w.append("", bold("Model Time Attribution"))
	appendTimeModelRows(w, rankedModelTimeUsage(overview.ModelTimeUsage), 5)
}

func appendTimeSourceLines(w *fittedLineWriter, rows []agentmodel.AgentTimeUsage) {
	rows = rankedAgentTimeUsage(rows)
	w.append("", bold("Source Time Comparison"))
	if len(rows) == 0 {
		w.append("No source time rows.")
		return
	}
	totalSessions := 0
	totalWall := int64(0)
	...
	appendTimeSourceRows(w, rows, 12)
}

func appendTimeSourceRows(w *fittedLineWriter, rows []agentmodel.AgentTimeUsage, limit int) {
	appendFittedLineSection(w, fittedRowSection[agentmodel.AgentTimeUsage]{
		empty: "No source time rows.",
		table: fittedRowTable[agentmodel.AgentTimeUsage]{
...
func appendTimeModelRows(w *fittedLineWriter, rows []agentmodel.ModelTimeUsage, limit int) {
	appendFittedLineSection(w, fittedRowSection[agentmodel.ModelTimeUsage]{
		empty: "No model time rows.",
		table: fittedRowTable[agentmodel.ModelTimeUsage]{
...
func appendTimeToolLines(w *fittedLineWriter, rows []agentmodel.ToolTimeUsage) {
	rows = rankedToolTimeUsage(rows)
	w.append("", bold("Tool Duration Leaders"))
	if len(rows) == 0 {
		w.append("No tool duration rows.")
		return
	}
	w.append(dim("Network-likely is inferred from tool names and shell/network activity."))
	appendFittedLineSection(w, fittedRowSection[agentmodel.ToolTimeUsage]{
		table: fittedRowTable[agentmodel.ToolTimeUsage]{
...
func appendTimeSessionLines(w *fittedLineWriter, rows []agentmodel.Session) {
	w.append("", bold("Slow Sessions"))
	if len(rows) == 0 {
		w.append("No slow sessions yet.")
		return
	}
	w.appendFit("  Started          Source       Model              Wall       Active     Model      Tool       Project / session")
	for _, session := range limitSlice(rows, 16) {
		w.appendFit(fmt.Sprintf("  %-16s %-12s %-18s %10s %10s %10s %10s  %s",
