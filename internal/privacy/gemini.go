...
func (a GeminiAdapter) Apply(settingIDs []string) (model.PrivacyConfigApplyResult, error) {
	...
	selected, warnings := selectedJSONSettingDefinitions(settingIDs)
...
func selectedJSONSettingDefinitions(settingIDs []string) ([]jsonSettingDefinition, []string) {
	if len(settingIDs) == 0 {
		return append([]jsonSettingDefinition(nil), geminiSettingDefinitions...), nil
	}
	...
	return selected, unknownJSONSettingWarnings(ids)
}

func unknownJSONSettingWarnings(ids map[string]struct{}) []string {
	if len(ids) == 0 {
		return nil
	}
	...
	for _, id := range unknown {
		warnings = append(warnings, fmt.Sprintf("unknown Gemini privacy setting %q was ignored", id))
	}
	return warnings
}
