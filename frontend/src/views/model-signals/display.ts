...
type DriftRecord = { drift?: { severity?: string } }
type ProjectSourceRecord = { projectPath?: string; rawSourcePath?: string }
type ConfidenceRecord = Pick<ModelSignalsDailyMetric, 'keyReason' | 'drift' | 'lowSample'>
...
export function createModelSignalsDisplay(t: ModelSignalsTranslate) {
  return {
    ...
    anomalyRowKey,
    cohortRowKey,
    confidenceReason: (record: ConfidenceRecord) => confidenceReason(record, t),
    dailyRowKey,
    displayPair,
    displayPercent,
    displayRate,
    ...
    formatConfidence: (value?: string | number) => formatConfidence(value, t),
    formatLatency,
    formatOptionalCost,
    formatPercent,
    formatPressure,
    formatRate,
    formatThroughput,
    formatWindow: (window?: ModelSignalsWindow) => formatWindow(window, t),
    matrixCellKey,
    matrixCellTitle: (cell: ModelSignalMatrixCell) => matrixCellTitle(cell, t),
    matrixRowKey,
    metricClass,
    normalizeAnomaly,
    p10Throughput,
    p90Latency,
    projectHealthTitle: (record: ProjectMetricRow) => projectHealthTitle(record, t),
    projectInfo,
    projectMixInfo: (record: ProjectMetricRow) => projectMixInfo(record, t),
    projectRowKey,
    reasonCount,
    reasonSeverity,
    reasonText,
    sessionInfo,
    severityClass,
    severityLabel: (value?: string) => severityLabel(value, t),
    severityMetricTone,
    severityRank,
    severityTagColor,
    sourceInfo: (record: Parameters<typeof sourceDisplay>[0]) => sourceInfo(record, t),
    unpricedNote: (metric?: ModelSignalMetricSet) => unpricedNote(metric, t)
  }
}
...
function unpricedNote(metric: ModelSignalMetricSet | undefined, t: ModelSignalsTranslate) {
  const count = metric?.unpricedSessionCount || 0
  return count > 0 ? `${formatNumber(count)} ${t('label.unpriced')}` : ''
}

function confidenceReason(record: ConfidenceRecord, t: ModelSignalsTranslate) {
...
function formatConfidence(value: string | number | undefined, t: ModelSignalsTranslate) {
...
function formatWindow(window: ModelSignalsWindow | undefined, t: ModelSignalsTranslate) {
...
function metricClass(current?: number, baseline?: number, lowerIsBetter = false) {
  ...
  if (isMetricDegraded(currentValue, baselineValue, lowerIsBetter)) return 'status-error'
  if (isMetricImproved(currentValue, baselineValue, lowerIsBetter)) return 'status-ok'
  return ''
}

function isMetricDegraded(current: number, baseline: number, lowerIsBetter: boolean) {
  if (lowerIsBetter) return current > baseline * 1.15
  return current < baseline * 0.85
}

function isMetricImproved(current: number, baseline: number, lowerIsBetter: boolean) {
  if (lowerIsBetter) return current < baseline * 0.9
...
function severityLabel(value: string | undefined, t: ModelSignalsTranslate) {
  const normalized = (value || 'ok').toLowerCase()
  const translationKey = severityTranslationKeys[normalized]
  return translationKey ? t(translationKey) : normalized
}

const severityTranslationKeys: Record<string, Parameters<ModelSignalsTranslate>[0]> = {
...
function driftRowClass(record: DriftRecord) {
...
function sourceInfo(record: Parameters<typeof sourceDisplay>[0], t: ModelSignalsTranslate) {
  return sourceDisplay(record, t('fallback.unknown'))
}

function projectInfo(record: ProjectSourceRecord) {
  return projectDisplay(record.projectPath || record.rawSourcePath)
}

function projectMixInfo(record: ProjectMetricRow, t: ModelSignalsTranslate) {
...
function projectHealthTitle(record: ProjectMetricRow, t: ModelSignalsTranslate) {
  return [
    `${t('column.health')}: ${severityLabel(record.drift?.severity, t)} (${formatConfidence(record.drift?.confidence, t)})`,
...
function matrixCellTitle(cell: ModelSignalMatrixCell, t: ModelSignalsTranslate) {
  return [
    ...
    `${t('column.confidence')}: ${formatConfidence(cell.confidence, t)}`
  ].join('\n')
}
