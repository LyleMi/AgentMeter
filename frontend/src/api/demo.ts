...
function filteredPromptSuggestions(filters: PromptSuggestionFilters = {}): PromptSuggestion[] {
...
function saveDemoPrompt(input: SavedPromptInput): SavedPrompt {
...
function updateDemoPrompt(id: number, input: SavedPromptInput): SavedPrompt {
...
function recordDemoPromptCopy(id: number): SavedPrompt {
  const index = savedPrompts.findIndex((item) => item.id === id)
  if (index < 0) throw new Error('Saved prompt not found')
...
export const demoApi: DemoApi = {
  ...
  listSavedPrompts: async () => clone(savedPrompts),
  ...
  ignorePromptSuggestion: async (suggestionKey) => {
    ignoredPromptSuggestionKeys.add(suggestionKey)
    return { ok: true }
  },
  unignorePromptSuggestion: async (suggestionKey) => {
