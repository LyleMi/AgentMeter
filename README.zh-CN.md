...
（cache）复用、日级/项目级效率指标、工具调用行为和模型健康/漂移信号，同时
终端界面复用同一份本地数据提供摘要视图。
...
- 按日期或项目看，每会话成本、每活跃小时成本、缓存节省、重试压力或失败压力
  是否发生变化？
- 真实使用中，哪些模型健康、服务漂移、吞吐、缓存或工具使用症状发生了变化？
- 哪些工具被调用得最多？
- 会话和工具调用分别耗时多久？
...
- 模型信号用于查看原始操作信号、日级/项目级运营效率和健康/漂移监控，包括
  输出扩展、reasoning 占比、缓存未命中、吞吐、可用时的模型调用状态、
  token/成本形态、每会话成本、每活跃小时成本、缓存节省、重试/失败压力、
...
1. 从 [Releases](https://github.com/LyleMi/AgentMeter/releases) 下载
   `AgentMeter-<platform>-<arch>` 压缩包，选择与你的系统和 CPU 架构匹配的版本。
2. 解压。
3. 运行 AgentMeter，启动本地 Web 仪表盘。
